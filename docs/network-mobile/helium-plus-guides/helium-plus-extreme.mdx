...
title: 'Extreme'
description:
  'A guide to setting up and testing your Extreme environment for use with the Helium Passpoint service.'
...
Use this guide to prepare an ExtremeCloud IQ deployment for Helium Passpoint: from initial setup
through testing.
...
<br />
3. Set an **SSID Name** for internal use (e.g. `Helium`) and a **Broadcast Name** for clients;
   these can match.
...
The controller will talk to your RadSecProxy; RadSecProxy then sends RADIUS over TLS to Helium.
Use the RadSecProxy host’s private IP or hostname from
[Deploy RadSecProxy Container](#deploy-radsecproxy-container).

Add a RADIUS server group and add your RadSec server(s) to it. Your Extreme controller must
...
8. In **IP/Host Name**, enter your RadSecProxy address and port `1812` (e.g.
   `192.168.1.100`). Click **Save IP Object**. The new server section now shows that IP/host
   object.
...
Put the following in a text file so you can paste it into the Supplemental CLI later. These
commands tie your network policy to Hotspot 2.0.
...
10. Click **Upload** (bottom right). The **Device Update** dialog opens.
    ...
    <br />
11. Under **Update Network Policy and Configuration**, choose **Complete Configuration Update**
    (not Delta).
